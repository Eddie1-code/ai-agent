...
public class MentorApp {
    ...
    private static final String SYSTEM_PROMPT =
            ...
            + "【结论】你对问题的判断和分析\n"
            + "【步骤】具体的分步行动方案，至少3步，每步一行，序号统一用「1. 」「2. 」格式（数字+英文句点+空格），直接写动作\n"
            + "【风险提醒】执行中需注意的风险、边界和注意事项\n"
            + "【复盘问题】1-3个引导用户自我反思的问题\n"
            + "如果信息不足，优先提出1-3个关键澄清问题。"
            + "避免空泛建议，不做医疗诊断、法律结论或投资承诺。"
            + "全文不要使用 ** 加粗符号，普通文字即可。"
            + "禁止编造不可验证的外部链接，尤其是图片URL。"
            + "当用户明确要求生成图片时，优先调用图片生成工具；若工具失败，清晰说明失败原因并给出可执行的替代提示词。"
            + "除非用户明确要求，否则不要主动引导用户执行生成PDF/导出PDF。";
    ...
    public Flux<String> doPlannerPlanByStream(String message, String chatId) {
        String plannerPrompt = """
                ...
                2) 五个二级标题必须逐字一致且每行必须以 ## 开头（禁止只写「一、目标理解」而不带 ##；每个 ## 标题必须独立成行、标题前空一行换行，禁止把标题粘在上一段正文末尾），例如：
                   ## 一、目标理解
                   ## 二、约束与风险
                   ## 三、计划策略
                   ## 四、具体方案
                   ## 五、复盘与下一步
                3) 全文不使用加粗 **：地点名直接写成「- 地点名：说明」，不要用 ** 包裹任何文字。
